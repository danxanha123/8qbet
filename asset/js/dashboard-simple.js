...
class SimpleDashboardManager {
    ...
    loadConfig() {
        const defaultConfig = {
            ...
            links: [
                { name: "Link1", showUrl: this.language === 'cn' ? "立即领取" : "NHẬN QUÀ NGAY", jumpUrl: "https://www.93375347.com/", icon: "asset/images/8q.png" },
                { name: "Link2", showUrl: this.language === 'cn' ? "值得信赖" : "ĐÁNG TIN CẬY", jumpUrl: "https://www.93375347.com/", icon: "asset/images/8q.png" },
                { name: "Link3", showUrl: this.language === 'cn' ? "高安全性" : "BẢO MẬT CAO", jumpUrl: "https://www.93375347.com/", icon: "asset/images/8q.png" },
                { name: "Link4", showUrl: this.language === 'cn' ? "快速提款" : "RÚT TIỀN NHANH", jumpUrl: "https://www.93375347.com/", icon: "asset/images/8q.png" }
            ]
        };
    ...
    saveConfig() {
        if (window.syncManager) {
            window.syncManager.saveConfig(this.currentConfig);
        } else {
    ...
    setupSync() {
        ...
        window.addEventListener('configUpdated', (e) => {
            if (e.detail && e.detail.config) {
                ...
                this.loadSavedData();
                this.showMessage(
                    this.language === 'cn' ? '配置已同步！' : 'Cấu hình đã được đồng bộ!', 
                    'success'
                );
            }
        });
    ...
    updateCurrentUser() {
        const elements = ['admin-username', 'current-username'];
        elements.forEach(id => {
